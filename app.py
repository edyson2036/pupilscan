"""
PupilScan Backend — Flask + ONNX Runtime (sin PyTorch)
Memoria estimada: ~150 MB (vs ~1.5 GB con ultralytics+torch)
"""
...
def analyze():
    ...
    try:
        ...
        log.info(f'Video: {len(video_bytes)/1024:.1f} KB')
...
def process_video(video_path):
    ...
    if not cap.isOpened(): return {'error': 'No se pudo abrir el video'}
    ...
    while True:
        ...
        try:
            ...
            mask, conf_val = postprocess_mask(outputs, orig_h, orig_w, CONF)
            if mask is not None:
    ...
    idx_fr   = idx_off + next((i for i,v in enumerate(seg_rec) if v>=thresh), len(seg_rec)-1)
    t_fr     = float(t_arr[idx_fr])
    Tc = round(t_min-T_LED_ON, 3); Ts = round(T_LED_OFF-t_min, 3); Tr = round(t_fr-T_LED_OFF, 3)
    det_pct   = round(detected/frame_idx*100, 1)
    conf_mean = round(float(np.mean(conf_list)) if conf_list else 0, 3)
    chart_b64 = build_chart(t_arr, areas_arr, pt_arr, T_LED_ON, T_LED_OFF, t_min, t_fr, Tc, Ts, Tr)
    rng = float(np.max(pt_arr)-np.min(pt_arr)) or 1.0
    return {
        'Tc':Tc,'Ts':Ts,'Tr':Tr,'p_base':round(p_base,1),'p_min':round(p_min,1),
        'det_pct':det_pct,'conf_mean':conf_mean,'total_frames':frame_idx,'detected':detected,
        't':t_arr.tolist(),'pt':pt_arr.tolist(),
        'pt_norm':((pt_arr-np.min(pt_arr))/rng).tolist(),
        't_norm':(t_arr/float(t_arr[-1])).tolist() if t_arr[-1]>0 else t_arr.tolist(),
        't_min':round(t_min,3),'t_fr':round(t_fr,3),'chart':chart_b64,
    }

def build_chart(t, raw, smooth, t0, toff, tmin, tfr, Tc, Ts, Tr):
    BG='#090e1a';BG2='#0f1828';CYAN='#00d4ff';GRN='#00ff9d';AMB='#ffb347';RED='#ff4d6d';DIM='#3d5280'
    fig, ax = plt.subplots(figsize=(9,4), facecolor=BG); ax.set_facecolor(BG2)
    ax.axvspan(0,t0,alpha=.10,color=CYAN); ax.axvspan(t0,tmin,alpha=.14,color=GRN)
    ax.axvspan(tmin,toff,alpha=.14,color=AMB); ax.axvspan(toff,tfr,alpha=.14,color=RED)
    ax.plot(t,raw,color=CYAN,alpha=.2,linewidth=.8); ax.plot(t,smooth,color=CYAN,linewidth=2)
    for tv,clr in [(t0,CYAN),(tmin,GRN),(toff,AMB),(tfr,RED)]:
        ax.axvline(tv,color=clr,linestyle='--',linewidth=.9,alpha=.7)
    y_ann = smooth.min()*.93
    for ta,tb,lbl,clr in [(t0,tmin,f'Tc={Tc:.2f}s',GRN),(tmin,toff,f'Ts={Ts:.2f}s',AMB),(toff,tfr,f'Tr={Tr:.2f}s',RED)]:
        ax.annotate('',xy=(tb,y_ann),xytext=(ta,y_ann),arrowprops=dict(arrowstyle='<->',color=clr,lw=1.2))
        ax.text((ta+tb)/2,y_ann*.96,lbl,ha='center',va='top',color=clr,fontsize=8,fontfamily='monospace')
    ...
    ax.grid(color='#1e2d4a',linewidth=.4,alpha=.6)
    patches=[mpatches.Patch(color=c,alpha=.5,label=l) for c,l in
             [(CYAN,'Basal'),(GRN,f'Tc={Tc:.2f}s'),(AMB,f'Ts={Ts:.2f}s'),(RED,f'Tr={Tr:.2f}s')]]
    ax.legend(handles=patches,fontsize=8,facecolor='#0f1828',labelcolor='#7a90b5',
              loc='upper right',framealpha=.8,edgecolor='#1e2d4a')
    plt.tight_layout()
    buf=BytesIO(); plt.savefig(buf,format='png',dpi=110,bbox_inches='tight',facecolor=BG)
